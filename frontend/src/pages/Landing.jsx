...
// Stitch tokens (#07090D, #2563FF, volt <5%, violet, radii 24/32, pill CTAs).
// No fake stats/personas/testimonials. CTAs only to /login, /signup, /dashboard.
...
function SectionHead({ number, eyebrow, title, body }) {
  return (
    <Reveal>
      <p className={kicker}>
        <span className="text-[#2563FF] dark:text-[#7DA6FF]">{number}</span>
        {'  /  '}
        {eyebrow}
      </p>
      <h2
        className="font-display font-bold tracking-tight text-balance text-[#0A0D12] dark:text-[#F5F7FA] mt-3"
...
/* ParticleButton (Kokonut pattern, inline): volt pill + CSS spark dots. */
function ParticleButton({ to, children }) {
  const reduced = useReducedMotion();
  return (
    <Link
      to={to}
      className="group relative inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-[#A7D700] text-[#0A0D12] font-semibold hover:brightness-95 transition overflow-visible"
    >
      {!reduced && (
        <span aria-hidden className="absolute inset-0 pointer-events-none">
          {[
            'left-4 -top-1 group-hover:-translate-y-2',
            'left-1/3 -top-1.5 group-hover:-translate-y-3 group-hover:translate-x-1',
            'right-1/3 -top-1 group-hover:-translate-y-2 group-hover:-translate-x-1',
            'right-5 -top-1.5 group-hover:-translate-y-3',
            'left-8 -bottom-1 group-hover:translate-y-2',
            'right-8 -bottom-1.5 group-hover:translate-y-3'
          ].map((pos, i) => (
            <span
              key={i}
              className={cn(
                'absolute w-1 h-1 rounded-full bg-[#A7D700] opacity-0 group-hover:opacity-100 transition-all duration-300',
                pos
              )}
            />
          ))}
...
/* Magnetic secondary CTA (editorial MagneticButton physics, anchor-safe:
   magnetism lives on a wrapper span so no interactives nest). */
...
function AreaShape() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" role="img" aria-label="Pipeline position curve across the six placement stages">
      ...
      {coords.map((c, i) => (
        <g key={i}>
          <circle cx={c[0]} cy={c[1]} r="3.5" fill="#07090D" stroke="#2563FF" strokeWidth="2" />
...
const ATTENDANCE_STATES = [
  { key: 'present', label: 'Present', color: '#2563FF', hint: 'Marked in the live matrix' },
  { key: 'od', label: 'On duty', color: '#A7D700', hint: 'Approved leave shapes' },
  { key: 'absent', label: 'Absent', color: '#7C5CFF', hint: 'Watch-list inputs' }
];
...
const JOURNEY = [
  ...
  { title: 'Follow the signals', body: 'AI reports attach evidence to every finding.', to: '/signup', cta: 'Create account' }
];
      ...
      <main>
        ...
        <section ref={heroRef} className="relative overflow-hidden" aria-label="Introduction">
          <div className="absolute inset-0 pointer-events-none" aria-hidden>
            ...
            />
            <div className="absolute inset-0 [mask-image:radial-gradient(ellipse_70%_60%_at_50%_35%,black,transparent)] bg-gradient-to-b from-[#2563FF]/[0.07] dark:from-[#2563FF]/[0.12] to-transparent" />
          </div>
          ...
          <motion.div style={reduced ? undefined : { y: previewY }} className="relative max-w-6xl mx-auto px-4 sm:px-6 pb-4">
        ...
        <section id="intelligence" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Academic intelligence">
          <SectionHead
            number="04"
          ...
          <div className="mt-7 grid lg:grid-cols-5 gap-3.5 sm:gap-4">
            <Reveal className="lg:col-span-3">
              <div className={`${glassCard} p-5 sm:p-6 text-[#0A0D12] dark:text-[#F5F7FA]`}>
            ...
            <Reveal delay={0.08} className="lg:col-span-2">
              <SpotCard className={`${glassCard} p-5 sm:p-6 h-full`}>
                ...
                <SplitReveal
                  className="font-display font-bold tracking-tight text-xl sm:text-2xl mt-3 text-[#0A0D12] dark:text-[#F5F7FA]"
                ...
                <ul className="mt-4 space-y-2.5">
                  {[
                    ...
                    <li key={text} className="flex gap-3 text-[13px] leading-relaxed text-[#4B5563] dark:text-[#A7B0BF]">
                      <span className="mt-0.5 w-8 h-8 shrink-0 rounded-full grid place-items-center bg-[#2563FF]/10 dark:bg-[#2563FF]/15 text-[#2563FF] dark:text-[#7DA6FF]" aria-hidden>
                        <Icon size={15} />
                      </span>
                      {text}
                    </li>
                ...
                <Link
                  to="/dashboard"
                  className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-[#2563FF] dark:text-[#7DA6FF] hover:underline underline-offset-4"
                >
        ...
        <section id="attendance" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Attendance">
          <SectionHead
            number="05"
            eyebrow="Attendance"
          ...
          <div className="mt-7 grid md:grid-cols-3 gap-3.5 sm:gap-4">
            ...
            <Reveal delay={0.16}>
              <div className={`${glassCard} p-5 sm:p-6 h-full flex flex-col`}>
                ...
                <Link
                  to="/dashboard"
                  className="mt-auto pt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[#2563FF] dark:text-[#7DA6FF] hover:underline underline-offset-4"
                >
        ...
        <section id="placement" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Placements">
          <SectionHead
            number="06"
            eyebrow="Placement"
          ...
          <div className="mt-7 grid lg:grid-cols-2 gap-3.5 sm:gap-4">
                      ...
                      <li key={s}>
                        <button
                          ...
                          className={cn(
                            ...
                            stage === i
                              ? 'border-[#2563FF]/60 bg-[#2563FF]/[0.08] dark:bg-[#2563FF]/15'
                              : 'border-black/10 dark:border-white/10 hover:border-[#2563FF]/40'
                          )}
            ...
            <Reveal delay={0.08} className="lg:sticky lg:top-24 self-start">
              <div className={`${glassCard} p-5 sm:p-6 min-h-[280px] flex flex-col`}>
                ...
                <Link
                  to="/placement"
                  className="mt-4 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#2563FF] text-white text-sm font-semibold hover:brightness-110 transition w-fit"
                >
        ...
        <section id="people" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="People">
          <SectionHead
            number="07"
            eyebrow="People"
          ...
          <div className="mt-7 grid sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
            {ROLES.map((role, i) => (
              <Reveal key={role} delay={Math.min(i * 0.06, 0.24)}>
                <SpotCard className={`${glassCard} p-5 flex gap-4 items-start h-full`}>
                  <span
                    className="w-12 h-12 shrink-0 rounded-2xl grid place-items-center font-display font-bold text-lg bg-[#2563FF]/10 dark:bg-[#2563FF]/15 text-[#2563FF] dark:text-[#7DA6FF]"
                    aria-hidden
                  >
                  ...
                  <span>
                    ...
                    <Link
                      to="/dashboard"
                      className="mt-2.5 inline-flex items-center gap-1 text-[13px] font-semibold text-[#2563FF] dark:text-[#7DA6FF] hover:underline underline-offset-4"
                    >
            ...
            <Reveal delay={0.24}>
              <Link to="/dashboard" className={`group ${glassCard} p-5 flex flex-col justify-center h-full hover:border-[#2563FF]/40 transition-colors min-h-[148px]`}>
                ...
                <span className="mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-[#2563FF] dark:text-[#7DA6FF]">
                  Enter directory
        ...
        <section id="ai-reports" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="AI reports">
          <SectionHead
            number="08"
            eyebrow="AI reports"
            ...
            <BeamCard className="p-px">
              <div className="rounded-[calc(1rem-1px)] p-5 sm:p-8">
                <p className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-black/10 dark:border-white/10 text-xs font-semibold text-[#4B5563] dark:text-[#A7B0BF]">
                  <Sparkles size={13} aria-hidden className="text-[#2563FF] dark:text-[#7DA6FF]" />
                  ...
                  {REPORT_CHAIN.map((r, i) => (
                    <li key={r.step} className="relative pl-8 pb-5 last:pb-0">
                      {i < REPORT_CHAIN.length - 1 && (
                        <span aria-hidden className="absolute left-[9px] top-6 bottom-0 w-px bg-[#2563FF]/25" />
                      )}
                      <span aria-hidden className="absolute left-1 top-1.5 w-[11px] h-[11px] rounded-full border-[2.5px] border-[#2563FF] bg-transparent" />
                      <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-[#2563FF] dark:text-[#7DA6FF]">
                        {r.step}
                      </p>
                      <p className="mt-1 text-[15px] leading-relaxed text-[#0A0D12] dark:text-[#F5F7FA] max-w-2xl">
                        {r.body}
                      </p>
                    </li>
                  ))}
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <Link
                    to="/signup"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#A7D700] text-[#0A0D12] text-sm font-semibold hover:brightness-95 transition"
                  >
                    Get reports <ArrowRight size={15} aria-hidden />
                  </Link>
                  <Link
                    to="/login"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-black/10 dark:border-white/15 text-sm font-semibold hover:border-[#2563FF]/50 transition-colors"
                  >
        ...
        <section id="roles" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Role experiences">
          <SectionHead
            number="09"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 mt-4">
            {ROLE_CARDS.map(({ Icon, tag, title, body, points }) => (
              <Reveal key={tag}>
                <Link
                  to="/login"
                  className={`group ${glassCard} p-6 flex flex-col gap-4 hover:border-[#2563FF]/40 dark:hover:border-[#2563FF]/50 transition-colors h-full`}
                >
        ...
        <section id="journey" className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Getting started journey">
          <SectionHead
            number="10"
            eyebrow="Journey"
          ...
          <div ref={journeyRef} className="mt-7 relative">
            <div aria-hidden className="absolute left-[21px] sm:left-[23px] top-2 bottom-2 w-[2px] bg-black/10 dark:bg-white/10 rounded-full">
              <motion.div
                className="w-full h-full origin-top bg-[#2563FF] rounded-full"
            ...
            <ol className="space-y-3.5 sm:space-y-4">
              {JOURNEY.map((j, i) => (
                <li key={j.title} className="relative ml-12 sm:ml-14">
                  <span
                    aria-hidden
                    className="absolute -left-12 sm:-left-14 top-5 w-[22px] h-[22px] sm:w-6 sm:h-6 rounded-full grid place-items-center font-mono text-[10px] font-bold bg-[#2563FF] text-white"
                  >
                    {i + 1}
                  </span>
                  <Reveal className={`${glassCard} p-5 flex flex-wrap items-center justify-between gap-3`}>
                    ...
                    <Link
                      to={j.to}
                      className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full border border-black/10 dark:border-white/15 text-sm font-semibold hover:border-[#2563FF]/50 transition-colors shrink-0"
                    >
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14" aria-label="Get started">
          <Reveal>
            <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[#0A0D12] px-6 py-16 sm:py-24 text-center">
              <div
                aria-hidden
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[720px] h-[420px] rounded-full bg-[#2563FF]/25 blur-[130px] pointer-events-none"
              />
  ...
  return PIPELINE_STAGES.map((s, i) => (
    <button
      ...
      className={cn(
        ...
        active === i ? 'w-7 bg-[#2563FF]' : 'w-2 bg-black/15 dark:bg-white/20 hover:bg-black/30 dark:hover:bg-white/40'
      )}
    />
  ));
}
